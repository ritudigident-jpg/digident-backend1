 ...
 * and manual-order invoices in one place, grouped by customerNo (the same
 * identity Invoice already uses internally via generateCustomerNo, so two
 * invoices for the same person always land in the same bucket regardless
 * of which flow created them).
 *
 *   totalReturnedValue     = value of everything ever recorded as returned
 *                             against the invoice
 *   pendingReturnRefund    = totalReturnedValue - amount already refunded
 *   cancellationRefundOwed = paidAmount, only when the invoice itself is
 *                             "cancelled" and that paid amount hasn't been
 *                             refunded back out yet
 *   unpaidDue              = amountToPay, only when the invoice is still
 *                             open (not cancelled) and something is owed
 *
...
export const getInvoiceCustomerLedgerService = async (query = {}) => {
  ...
  pipeline.push({
    $project: {
      ...
      customerPhone: 1,
      customerEmail: 1,
      // Aliased to match the shape the frontend already expects
      // (previously fed by the ManualOrder-only ledger).
 ...
 * method === "credit_note" — i.e. every "customer will take it next time"
 * settlement, no matter which of the 3 invoice-creation flows the source
 * invoice came from.
 */
